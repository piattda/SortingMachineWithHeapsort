...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if(last - top > 1){
            if(order.compare(first, array.entry(left))>0 || order.compare(first, array.entry(right))>0){
            
            //it has a right child then
                //2 children
                //compare last and its children 
    ...
    private static <T> void heapify(Array<T> array, int top, Comparator<T> order) {
        ...
        /*
         * both sub trees must be heaps. then call sift down. 
         * 
         * sub trees are made into heaps by?
         *  
         * possibly everything in the array is not sorted. So go down to leaf nodes and call siftDown
         * 
         * 
         * 
         * if right <= last
         *  heapify left and right
         *
         * else
         *  heapify left
         *  
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        /*
         * first value is root. 
         * second is second spot
         * third next node
         * 
         */
        
        Array<T> newArray = new Array1L(q.length());
        int counter = 0;
        while(q.length()>0){
            newArray.setEntry(counter, q.dequeue());
            counter ++;
        }
        heapify(newArray, 0, order);
        return newArray;
    }
    ...
    public final int size() {
        ...
        assert this.conventionHolds();
        int toReturn = 0;
        //in insertion mode size in queue
        //in extraction mode we use heapSize
        if(this.insertionMode){
            
            toReturn = this.entries.length();
            
        }else{
            toReturn =this.heapSize; 
        }
        return toReturn;
    }
